function listBooks(req, res) {
  ...
  this.Book.find(
    {},
    {},
    { lean: true, limit: 10, skip: page * 10 },
    (err, docs) => {
      if (err) {
        res.json({ erro: err });
      } else {
        const _q = docs.map((doc, i) => {
          return this.Exemplar.find({ book_id: doc._id }, '_id loaned')
            .exec()
            .then(exemplars => {
...
function listExemplars(req, res) {
  this.Book.findById(req.params.book_id, {}, { lean: true })
    .exec()
    .then(book => {
      if (!book)
        return res.status(404).json({ error: 'Livro não cadastrado.' });
      this.Exemplar.find({ book_id: req.params.book_id }, '-book_id')
        .exec()
        .then(docs => {
          book.exemplars = docs;
          res.json(book);
